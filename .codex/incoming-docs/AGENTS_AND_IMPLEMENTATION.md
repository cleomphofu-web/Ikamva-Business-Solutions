...
- [ ] Thread memory (thread_id in memory rows)
- [ ] Startup recovery for orphaned tasks

## Phase 3 — Autonomy & Safety (Next)
- [ ] `autonomy_mode` column on `ai_employees`
- [ ] Action risk classification config (`action-risk.js`)
- [ ] WorkerEngine enforces risk class + autonomy mode before every external action
- [ ] Extended lifecycle states (`needs_attention`, `blocked`, `testing`, `setup_required`)
- [ ] Confidence scoring on approval cards
- [ ] "Edit and approve" on approval cards
- [ ] Low-confidence proactive escalation notification

## Phase 4 — Data Integrity
- [ ] localStorage fully retired (invoices, projects, tasks, services)
- [ ] Per-tenant rate limiting (`tenant_usage_windows`)
- [ ] Webhook trigger endpoint
- [ ] Email verification re-enabled with dev bypass
- [ ] Postgres advisory lock on triage task creation (prevents duplicate chains)

## Phase 5 — UX Completeness
- [ ] Live chain thinking display in Mission Control Overview
- [ ] Dashboard: 3-panel first view (approvals, activity, Employee health)
- [ ] Employee workspace page (dedicated page per Employee with all tabs)
- [ ] Integration connected state in onboarding Skills step
- [ ] Upgrade prompt after first completed value delivery
- [ ] Rotating headline on landing page
- [ ] Fast-track intent parser in setup
